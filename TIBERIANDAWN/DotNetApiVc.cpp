...
{
	IO::File::AppendAllText("DotNetDbg.txt", gcnew String(str));
	IO::File::AppendAllText("DotNetDbg.txt", Environment::NewLine);
	
	/*const auto mStr = gcnew String(str);
	System::Windows::Forms::MessageBox::Show(mStr);		*/
}

void DotNetApiVc::DebugBox(int str)
{
	IO::File::AppendAllText("DotNetDbg.txt", str.ToString());
	IO::File::AppendAllText("DotNetDbg.txt", Environment::NewLine);
}
